        ...
        GeometryReader { geo in
            ...
            ZStack(alignment: .topLeading) {
                ...
                Text("즐겨찾기")
                    ...
                    .offset(x: 168 * scale, y: 74 * scale)

                Text("관심 도서 \(books.count)권 · 대출 가능해지면 알려드려요")
                    ...
                    .offset(x: 24 * scale, y: 116 * scale)

                VStack(spacing: 16 * scale) {
                    ForEach(books, id: \.self) { book in
                        FavoriteBookRow(title: book, scale: scale) {
...
struct FavoritesView_Previews: PreviewProvider {
    static var previews: some View {
        FavoritesView().previewDevice("iPhone 16")
    }
}
