...
public struct RankingView: View {
    ...
    private func podium(scale: CGFloat) -> some View {
        ZStack(alignment: .bottomLeading) {
            podiumPerson(name: "김길동", count: "46 권", rank: 2, height: 76, color: [Color(red: 240/255, green: 240/255, blue: 243/255), Color(red: 221/255, green: 223/255, blue: 230/255)], scale: scale).offset(x: 0 * scale, y: 0)
            podiumPerson(name: "홍길동", count: "49 권", rank: 1, height: 96, color: [Color(red: 226/255, green: 236/255, blue: 205/255), Color(red: 199/255, green: 220/255, blue: 161/255)], scale: scale).offset(x: 116 * scale, y: 0)
            podiumPerson(name: "이길동", count: "45 권", rank: 3, height: 62, color: [Color(red: 243/255, green: 233/255, blue: 221/255), Color(red: 228/255, green: 210/255, blue: 190/255)], scale: scale).offset(x: 232 * scale, y: 0)
        }.frame(width: 332 * scale, height: 222 * scale, alignment: .bottomLeading)
    }
    private func podiumPerson(name: String, count: String, rank: Int, height: CGFloat, color: [Color], scale: CGFloat) -> some View {
        VStack(spacing: 3 * scale) {
            ProfileAvatar(size: rank == 1 ? 64 : 52, scale: scale)
            Text(name).font(FeatureFontFamily.Pretendard.bold.swiftUIFont(size: 14 * scale))
            Text(count).font(FeatureFontFamily.Pretendard.medium.swiftUIFont(size: 12 * scale)).foregroundColor(rank == 1 ? FeatureAsset.Color.buttonColor.swiftUIColor : Color(red: 154/255, green: 154/255, blue: 161/255))
    ...
    private func rankingList(scale: CGFloat) -> some View {
        VStack(spacing: 0) {
            RankingListRow(rank: 4, name: "정길동", detail: "2학년 · 소프트웨어 개발과", count: "31권", scale: scale)
            Divider().padding(.horizontal, 21 * scale)
            RankingListRow(rank: 5, name: "최길동", detail: "1학년 · AI과", count: "28권", scale: scale)
            Divider().padding(.horizontal, 21 * scale)
            RankingListRow(rank: 6, name: "한길동", detail: "2학년 · 소프트웨어 개발과", count: "20권", scale: scale)
